...
namespace NewBosonT
{
    class patternsparser
    {
            ...
            char[] chArray1 = new char[2]
      {
        ...
        public void addRow(int count, DataGridView dgv)
        {
            commitNewRow(dgv);
            for (int i=0; i != count; i++)
            {
                DataGridViewRow row = new DataGridViewRow();
                dgv.Rows.Insert(dgv.SelectedCells[0].RowIndex, row);
            }
            //dgv.FirstDisplayedScrollingRowIndex = dgv.Rows.Count - 1;
        }
        public void removeRow(DataGridView dgv)
        {
        ...
        public void countCells(Label lbl, DataGridView dgv)
        {
            int cellcount = dgv.SelectedCells.Count;
            lbl.Text = cellcount.ToString() + " platforms selected. " /* + rowcount.ToString() + " rows selected. " + colcount + " columns selected."*/;
        }

        public void changeCellValue(string type, DataGridView dgv, ListView lvw, ComboBox cbx, TextBox txt)
        {
            ...
            switch (type)
            {
                ...
                case "*":
                    IEnumerator enumerator16 = dgv.SelectedCells.GetEnumerator();
                    try
                    {
                        while (enumerator16.MoveNext())
                        {
                            DataGridViewCell dataGridViewCell = (DataGridViewCell)enumerator16.Current;
                            if (dataGridViewCell.ColumnIndex == 10)
                            {
                    ...
                    {
                        IDisposable disposable = enumerator16 as IDisposable;
                        if (disposable != null)
                            disposable.Dispose();
                    }
                    break;
        ...
        public void buildLuaNouveau(string luapath, ListView lvw)
        {
            string totaltext = "return {\r\n";

            foreach (ListViewItem item in lvw.Items)
            {
                string[] lines = item.Tag.ToString().Split(new string[] { "\n" }, StringSplitOptions.None);
                int n = 0;
                foreach (string line in lines)
                {
                    if (n == 0)
                    {
                        ...
                        if (firstlinedata[0] == "true")
                        {
                            totaltext += "noenergy = true, ";
                        }
                        //MessageBox.Show(firstlinedata[1].Trim());
                        if (firstlinedata[1].Trim() != String.Empty)
                        {
                            totaltext += "stages = {" + firstlinedata[1] + "}, ";
                        }
            ...
            File.WriteAllText(luapath, totaltext);
        }
        public void addSection(TextBox txt, ListView lvw)
        {
            if (txt.Text.Trim() != "")
            {
